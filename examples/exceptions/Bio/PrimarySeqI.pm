...
=head1 SYNOPSIS
	...
	$obj->throw(-class => 'Bio::Root::Exception',
                    -text => "Could not reverse complement. Probably not DNA. Actual exception\n$@\n",
                    -value => $@);
    }
...
sub trunc{
   ...
   if( !$end ) {
	   ...
	   $self->throw(-class => 'Bio::Root::BadParameter',
                        -text => "End not specified. Usage: trunc(start,end)");
       } else {
	   confess("End not specified. Usage: [$self] trunc start,end");
       }
   }
   ...
   if( $end < $start ) {
       if( $self->can('throw')  ) {
	   $self->throw(-class => 'Bio::Root::BadParameter',
                        -text => "$end is smaller than $start. if you want to truncated and reverse complement, you must call trunc followed by revcom. Sorry.",
