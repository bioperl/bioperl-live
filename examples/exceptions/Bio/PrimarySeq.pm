...
sub new {
	...
	if( $id ne $given_id ) {
	    $self->throw(-class => 'Bio::Root::BadParameter',
                         -text => "Provided conflicting id and display_id constructor functions. [$id] [$given_id]",
   ...
   if( defined $value) {
	  ...
	  ($value !~ /^[A-Za-z\-\.\*]+$/)) {
	   $obj->throw(-class => 'Bio::Root::BadParameter',
                       -text => "Attempting to set the sequence to [$value] which does not look healthy",
                       -value => $value);
       }
...
sub subseq {
   ...
   if( $start > $end ){
       $self->throw(-class => 'Bio::Root::BadParameter',
                    -text  => "in subseq, start [$start] has to be less than end [$end]",
                    -value => "start=$start, end=$end");
   }

   if( $start <= 0 || $end > $self->length ) {
       $self->throw(-class => 'Bio::Root::BadParameter',
                    -text => "You have to have start positive and end less than the total length of sequence [$start:$end] Total ".$self->length."",
                    -value => "start=$start, end=$end");
   }

   # since sequences are indexed starting at 1,
...
sub moltype {
	...
	unless ( $valid_type{$value} ) {
	    $obj->throw(-class => 'Bio::Root::BadParameter',
                        -text => "Molecular type '$value' is not a valid type (".
...
sub _guess_type {
   ...
   if( $total == 0 ) {
       $self->throw(-class => 'Bio::Root::BadParameter',
                    -text => "Got a sequence with no letters in - cannot guess type [$str]",
                    -value => $str);
   }
