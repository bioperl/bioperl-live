...
=head2 _give_tracesbases_binary
...
sub _give_tracesbases_binary {
    ...
    # now populate them
    my ($current_base,$place_base_at,$peak_quality,
	$ramp_counter,$current_ramp,$ramp_position);
    ...
    ($samples->{'strings'}->{'trace_string'}->{'string'},
     ...
     $samples->{'arrays'}->{'traces'}) = 
	 &_make_trace_string($samples->{'arrays'}->{'sam_a'},
			     $samples->{'arrays'}->{'sam_c'},
