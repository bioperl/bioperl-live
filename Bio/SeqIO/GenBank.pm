...
#
# This code is based on the Bio::SeqIO::GCG module with
# the necessary alterations needed to get it to read
# and write GenBank formatted sequences made by
# kai kumpf <kku@imb-jena.de>
#
#
#-----------------------------------------------------------------------------
...
Bio::SeqIO::GenBank - Raw sequence input/output stream
...
This object can transform Bio::Seq objects to and from GenBank-formatted
sequence files. 
...
=head1 AUTHORS - Ewan Birney & Chris Dagdigian & Kai Kumpf

Email birney@sanger.ac.uk, dag@sonsorol.org kku@imb-jena.de

Ewan Birney wrote the all the base SeqIO stuff. This module is based
on the his SeqIO::GCG and has been tweaked by Kai Kumpf,
to support GenBank formatted sequence input/output.
...
sub next_seq{
   ...
   my ($seq,$fh,$line,$sequence,$chksum,$definitionon);
   ...
   LINE: while( <$fh> ) {
    ...
    /^LOCUS\s+(\w+)\s+(\d+)\s+bp\s+(\w+)\s+(\w+)\s+(\d{2}-\w{3}-\d{4})/ && do {
     $type=$3;
    };

    (/^DEFINITION\s+(.+)/ || ($definitionon && /^\s{12}(\w+.*)/)) && do {
     $desc .= $1;
     $definitionon = 1;
    };
    ...
    /^ORIGIN/ && do {
      ...
      $line = uc($line);            ## uppercase sequence
      $sequence .= $line;
      eof $fh && last LINE;
     }
...
sub write_seq {
   ...
   #Generate the GenBank Checksum value
   for($i=0; $i<$len ;$i++) {             
       $cnt++;
