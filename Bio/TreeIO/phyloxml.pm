...
sub add_attribute {
    ...
    push @{ $self->{'_lastitem'}->{'current'} },
      { 'dummy' => {}
      };    # current holds current element and empty hash for its attributes
...
# Bio::Tree has nodes and possibly other bits, need to map these 
...
sub _clade_els {
    ...
    if ( $tree_node->has_sequence ) {
        for my $seq ( @{ $tree_node->sequence } ) {
            ...
            # TODO: missing the attributes here?
...
sub _print_relations {
    ...
    my $rel_el = $helper->create_node(
        {
            ...
            Data    => {
                Tag => $rel->tagname,
                Attributes  => {
                    id_ref_0    => $id_ref_0->value,
                    id_ref_1    => $id_ref_1->value,
                    type        => $rel->type
                }
    ...
    if ($confidence) {
        ...
        $confidence_el->setAttribute('type', $confidence_type) if defined($confidence_type);
    }
}
...
sub _read_annotation_text_Helper {
    ...
    foreach my $ac (@$acs) {
        foreach my $ann ( $ac->get_Annotations($key) ) {
            if ( $ann->isa('Bio::AnnotationCollectionI') ) {
                push( @nextacs, $ann );
            }
        }
    }
...
sub element_clade {
    ...
    foreach my $tag ( keys %clade_attr ) {
        my $sv =
          Bio::Annotation::SimpleValue->new( -value => $clade_attr{$tag} );
        $newattr->add_Annotation( $tag, $sv );
    }
...
sub element_relation {
    ...
    {
        $self->throw( "id_ref and id_src incompatible: $id_ref_0, $id_ref_1, ",
...
sub element_default {
    ...
    if (   ( $srcbyidref && $srcbyidref->isa( $self->nodetype ) )
        || ( ( !$srcbyidref ) && $prev eq 'clade' ) )
    {
...
sub end_element_default {
    ...
    # we are annotating a Node
    elsif (( $srcbyidref && $srcbyidref->isa( $self->nodetype ) )
        || ( ( !$srcbyidref ) && $prev eq 'clade' ) )
    {
        ...
        if ( $current eq 'name' ) {
            ...
            if (   ( exists $self->current_attr->{'type'} )
                && ( $self->current_attr->{'type'} eq 'bootstrap' ) )
            {
                $tnode->bootstrap( $self->{'_currenttext'} )
                  ;    # this needs to change (adds 'B' annotation)
            }
        }
...
sub annotateNode {
    ...
    if ( $self->{'_currenttext'} ) {
        my $newvalue = Bio::Annotation::SimpleValue->new(
            -value => $self->{'_currenttext'} );
