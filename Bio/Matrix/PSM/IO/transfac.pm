...
=head2 new
 ...
 Title   : new
 Usage   : my $psmIO =  new Bio::Matrix::PSM::IO(-format=>'transfac', -file=>$file);
...
my ($line,@a,@c,@g,@t, $id, $tr1, $accn, $bf, $sites);
my $i=0;
...
while (my $line=$self->_readline) {  #How many sites?
	if ($line=~/^BA\s/) {
...
sub _make_matrix {
	...
	for (my $i=0; $i<@{$cA}+1;$i++) {
		if ((${$cA}[$i]+${$cC}[$i]+${$cG}[$i]+${$cT}[$i])==0) {
			...
			push @g,${$cG}[$i];
			push @t,${$cT}[$i];
			$ave = ((${$cA}[$i]+${$cC}[$i]+${$cG}[$i]+${$cT}[$i])/4 +$ave)/2;
		}
	}	
	for (my $i=0; $i<$#a;$i++) {
