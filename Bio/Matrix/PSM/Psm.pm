...
# Let the code begin...
 ...
 use vars qw(@ISA);
 use strict;
 @ISA=qw(Bio::Matrix::PSM::psmI Bio::Matrix::PSM::SiteMatrix Bio::Root::Root);
...
sub matrix {
	my $self = shift;
	my $prev = new Bio::Matrix::PSM::SiteMatrix(-pA=>$self->{probA}, -pC=>$self->{probC},-pG=>$self->{probG},-pT=>$self->{probT},
												-IC=>$self->{IC},-e_val=>$self->{e_val},-id=>$self->{id});
	if (@_) {
		my $matrix=shift;
		$self->{IC} = $matrix->IC;
