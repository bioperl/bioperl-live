...
$in  = Bio::SeqIO->new(-file => "contig1.fa", '-format' => 'fasta');

my $seq = $in->next_seq();

my @feats = $factory->mask($seq); #return an array of Bio::SeqFeature::FeaturePair objects
...
my $masked_seq = $factory->masked_seq; #return the masked sequence, a Bio::SeqI object

    
=head1 DESCRIPTION
RepeatMasker is a program that screens DNA sequences for
interspersed repeats known to exist in mammalian genomes as well as
for low complexity DNA sequences. For more information, on the program and its usage,
...
sub new {
  ...
  unless ($self->exists_rm()) {
    if( $self->verbose >= 0 ) {
      warn "RepeatMasker program not found as ".$self->program." or not executable. \n"; 
    }
  }
...
=head2  exists_rm()
 ...
 Function: Determine whether RepeatMasker program can be found on current host
 Example :
...
=head2  mask
 ...
 Example :
 Returns : an array of repeat features that are Bio::SeqFeature::FeaturePairs
...
=head2  _parse_results()
 ...
 Function:   parses the results from RepeatMasker output (largely copied from Ensembl RepeatMasker Runnable)
 Example :
...
sub _setinput {
  my ($self,$seq) = @_;
  $seq->isa("Bio::PrimarySeqI") || $self->throw("Need a Bio::PrimarySeq compliant object for RepeatMasker");
