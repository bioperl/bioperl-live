...
generated by TIGR's Lucy program.
...
There are a few methods that will only be available if you make some minor changes to
the source for Lucy and then recompile.  
The changes are in the 'lucy.c' file and there is a diff between the original and 
the modified file available at: www.angelfire.com/linux/bioinformatics/perl/lucy.diff
...
=head2 Creating a Lucy object
  ...
  @params = ('seqfile' => 'lucy.seq', 'adv_stderr' => 1, 'fwd_desig' => '_F', 'rev_desig' => '_R');
  $lucyObj = Bio::Tools::Lucy->new(@params);

=head2 Using a Lucy object

  You should get an array with the sequence names in order to use accessor methods. 
  Note:  The Lucy binary program will fail unless the sequence names provided 
  as input are unique.

  $names_ref = $lucyObj->get_sequence_names();

  This code snippet will produce a Fasta format file with sequence lengths and %GC 
  ...
  Print seq and qual values for sequences >400 bp in order to assemble them with CAP3 
  (or other assembler).
  ...
  Get all the sequences as Bio::PrimarySeq objects (eg., for use with Bio::Tools::Blast 
  to perform BLAST).

  @seqObjs = $lucyObj->get_Seq_Objs();

  Or use only those sequences that are full length and have a Poly-A tail.
  ...
  Print the names of the rejects and 1 letter code for reason they were rejected.
  ...
  There is a lot of other information available about the sequences analyzed by Lucy 
  (see APPENDIX).  This module can be used with the DBI module to store this sequence
...
If you don't provide filenames for qualfile, infofile or stderrfile, the module will assume 
that .qual, .info, and .stderr are the file extensions and search in the same directory 
as the .seq file for these files.  

For example, if you create a Lucy object with $lucyObj = Bio::Tools::Lucy->new(seqfile => lucy.seq), 
the module will find lucy.qual, lucy.info and lucy.stderr.

You can omit any or all of the quality, info or stderr files, but you will not be able to use 
all of the object methods (see method documentation below). 	   
