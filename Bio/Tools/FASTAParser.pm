...
#Important Notice: currently this module can only analyze FASTA output generated by redirection (not
#generated interactively) or using -q parameter. It can only parse single-threaded FASTA version 3.3 output under Unix/Linux.
...
FASTAParser is a package for parsing FASTA reports. The FASTA programs (including FASTA, FASTX,
FASTY, TFASTX, TFASTY and SSEARCH) are a family of widely used algorithms
for sequence database similarity searches. There are differences in various versions of FASTA
using various parameters under different operating system and hardware system.

This parser is far from perfect and I am expecting the users will find enomous bugs. Anyway, there
is no FASTA parser in BioPerl project so I think it is better to get an imperfect parser than not.

It is written to run standalone (independent of BioPerl). In the future I can of course integrate
it into BioPerl framework if needed.

Do not hesitate to report bugs to me. Alternatively, if you have a better parser, please upload into
repository to override mine.

=head2 Object

To create a new report, you pass a filehandle reference to the FASTAParser constructor.
...
A subject is a FASTA hit or a MSP, which should not be confused with an HSP in BLAST.

Each subject has about 20 attributes and 1 method to print out all attributes.
Some examples:
