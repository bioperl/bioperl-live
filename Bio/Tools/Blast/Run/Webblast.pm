...
This module is wrapped by the B<Bio::Tools::Blast.pm> module
and is intended for use via that module. This module can be used
...
See L<blast_remote>() for a description of available parameters.
...
Bio::Tools::Blast::Run::Webblast.pm executes a Blast request on an NCBI server 
by mimicking a web (http) client.
This module exports a single function L<blast_remote>(). See the docs for that method
for detailed information. 

Webblast.pm is derived from the stand alone B<webblast>
script by Alex Dong Li:

    http://www.genet.sickkids.on.ca/bioinfo_resources/software.html#webblast

=head1 DEPENDENCIES

B<Bio::Tools::Blast::Run::Webblast.pm> is used by B<Bio::Tools::Blast.pm>
and requires B<postclient.pl> for submitting requests Blast
server. This script is included with the distribution package. 
B<postclient.pl> should be located in the same directory as 
Bio::Tools::Blast::Run::Webblast.pm.

B<Bio::Tools::Blast::Run::Webblast.pm> also uses B<Bio::Seq.pm>
objects. The development of this module is somewhat linked with
Bio::Tools::Blast.pm module and should be updated along with that
module.
...
=item * Support for other Blast servers.

While in theory this module can be used to run a Blast on any remote server, 
it is currently specialized for the NCBI (or NCBI-conforming) servers.
The main source of this dependency is the configuration of options sent 
to the server (see L<_blast>()). There may also be some dependency in the processing
of the results files returned from the server, although I have tried to remove 
these (sac).

=item * PSI-Blast support.

PSI-Blast is not supported. Using a C<-version =E<gt> 'psi'> parameter will 
appear to execute properly but in the resulting file, only the header is obtained. 

=item * Reply by e-mail.

Reply by e-mail is supported but will generate an error since there is no
associated file containing the Blast results.
E-mail reply is probably not appropriate in this module context
since we are typically parsing the results immediately after the run
and we therefore need access to the file. Still, e-mail could be used
if necessary.
...
=item * Handle running PSI-Blasts.

=item * Retrieve gif files by getclient (for graphical overview).

=item * Bring up-to-date with latest changes in the forms used by the NCBI servers.

=item * Enable blast_remote() to handle lists of sequence filenames in addition to Bio::Seq.pm objects.
...
## SAC: 
## Automatically determine the path for postclient.pl
## (relative to the location of webblast)
my $pathOfPostclient='';
my $path = __FILE__;
if($path =~ /(.+\b)Webblast\.pm/) {
    $pathOfPostclient = $1.'postclient.pl';
} else {
    croak "\nWebblast can't set path for postclient.pl.\n";
}
...
@Blast_matrix_remote  = qw(BLOSUM62 PAM40 PAM120 PAM250 IDENTITY);
...
my $program                 ='blastp';
my $version                 = 2;
my $gappedAlignmentOption   ='GAPPED_ALIGNMENT=is_set';
...
my $graphicalOverviewOption =''; # or 'OVERVIEW=is_set';     # not available for email
my $graphicalOverviewFlag   ='off';
...
#      Most have the same names as Alex's. Some 
    my (@param) = @_;
...
#      default is gapped. Need to specify ungapped.
	...
	$gappedAlignmentOption="UNGAPPED_ALIGNMENT=is_set";
    }
    
    if ($NCBI_giFlag=~m/^on$/i) {
	$NCBI_giOption="NCBI_GI=is_set";
    } else {
	$NCBI_giOption="";
    }
    
    if ($graphicalOverviewFlag=~m/^on$/i) {
	$graphicalOverviewOption="OVERVIEW=is_set";
    } else {
...
# with html tags
	...
	$resultTypeOption="HTML=is_set";         # or html
    }
...
# if database is not of one available databases, quit
    ...
    # SAC: we are now loading the queue via Blast.pm.
...
# SAC: Some alterations in the basic plan of this function.
	...
	if (open(OUTPUT_FILE_TEMP, ">$outputFileNameTemp")==0 ) {
	    ...
	    $options = "-u \"http:\/\/$blastServerURL\" -f PROGRAM=$program DATALIB=$database INPUT_TYPE=\"sequence in fasta format\" SEQUENCE=\"$sequenceInFastaFormat\" EXPECT=$expect FILTER=$filter $NCBI_giOption DESCRIPTIONS=$descriptions ALIGNMENTS=$alignments $graphicalOverviewOption GENETICS_CODE=$queryGeneticCode OTHER_ADVANCED=\"$advancedOptions\" MATRIX=$matrix";
...
#		$MONITOR && print STDERR "Cleaning the file a bit...\n";
	    ...
	    }  else {  # by email (no results file to process.)
