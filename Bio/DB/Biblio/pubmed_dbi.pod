...
Bio::DB::Biblio:pubmed_dbi - Biblio access module for PubMed data
stored in relational databases accessed using DBI
...
the I<Bio::Biblio> module.  
  ...
  use DBI;
  use Bio::Biblio;
  my $dbh=DBI->connect('DBI:Sybase:server=our_server;database=my_database',
                       'my_username','my_password');
  my $biblio = new Bio::Biblio (-access => 'pubmed_dbi',
                                -dbh => $dbh,
...
Tied hash and tied filehandle interfaces also exist.
  ...
  tie %biblio,'Bio::DB::Biblio::pubmed_dbi',
	      -access => 'pubmed_dbi',
              -dbh=>$dbh,
              -collection=>'test_dbi';
  ...
  my $fh = newFh Bio::Biblio (-access => 'pubmed_dbi',
                              -dbh => $dbh,
			      -collection => 'genome papers'	);
  while(my $ref=<$fh>) {
    # process reference
...
All constructor forms (new, newFh, and tiehash) can open a dbh
internally, instead of requiring that one be passed in.  Here's an
example

  my $biblio = new Bio::Biblio (-access => 'pubmed_dbi',
                                -dsn => 'DBI:Sybase:server=our_server;database=my_database',
                                -user => 'my_username',
...
=head2 Constructors (call via Bio::Biblio)
 ...
 Usage   : $biblio=new Bio::Biblio(-access=>'pubmed_dbi',
                                   -dbh=>$dbh,
                                   -collection => 'genome papers')
 ...
 Args    : -access     Must be 'pubmed_dbi' for this module
           -collection Name of collection to open. Max 255 characters
           ...
           -dsn        DBI data source string of the form 
                       ...
                       Exactly one of -dsn or -dbh should be specified
           -user       User name valid for database
           -password   User's password for database
           -read_only  If true, collection is opened in read only mode
           -create     If true, collection is created even it already exists
                       If undef, collection is created if it does not 
                       exist
                       If defined but false, database is not created

 Notes   : Inherited from Bio::Biblio as extended by Root
 Notes   : Exactly one of -dsn or -dbh should be specified
...
one.
 ...
 Title  : tie
 Usage  : tie %biblio,'Bio::DB::Biblio::pubmed_dbi',
	              -access => 'pubmed_dbi',
                      -dbh=>$dbh,
                      -collection=>'test_dbi';
          my $ref=$biblio{12180136};
 ...
 Title   : newFh
 Usage   : $fh=newFh Bio::Biblio(-access=>'pubmed_dbi',
                                 -dbh=>$dbh,
                                 -collection => 'genome papers');
           while(my $ref=<$fh>) {
             # process reference
...
$object->xxx($new_value); To clear it, say $object->xxx(undef);
 ...
 Attr    : access
 Function: must be pubmed_dbi -- generally set via 'new'
 Access  : read-only 
