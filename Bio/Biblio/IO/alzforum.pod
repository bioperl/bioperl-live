...
Bio::Biblio::IO::alzforum - Biblio parsing module for Alzforum citation data
...
recommended to access it through I<Bio::Biblio::IO>.
  ...
  use Bio::Bublio;
  use Bio::Biblio::IO;
  my $biblio = new Bio::Biblio (-data_source => 'alzforum',
                                -query => TBD,
                                -auto_parse => 0);
  while (my $xml_abstract = $biblio->get_next) {
    my $reader = new Bio::Biblio::IO (-format=> 'alzforum');
    my $citation = $reader->parse($xml_abstract)
    # process citation further
  }
...
At present, this class can only format Alzforum citations.
The results are
...
$object->xxx($new_value); To clear it, say $object->xxx(undef);
 ...
 Attr    : what
 Function: What kind of data is being processed
