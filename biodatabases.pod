...
schema (http://obda.open-bio.org/), below. The most recent version of the
bioperl-db package can be obtained at 
http://cvs.open-bio.org/cgi-bin/viewcvs/viewcvs.cgi/?cvsroot=bioperl. 

The current version relies on the prior installation of a database
server, specifically Mysql, Postgres, or Oracle. The bioperl-db and 
BioSQL packages integrate neatly with Bioperl's objects and contain tables
for sequences, entries, sequence features, taxonomic information, 
references, keywords, a wide variety of genetic markers, and more. 
With a Seq object and a built-in database "adaptor" one can load the 
object's data in one step, as in 
