## $Id: biodatabases.pod,v 1.1 2002-02-05 20:34:13 bosborne Exp $
...
Bioperl can work with a number of database formats and allows users
...
Other layers above flat file indexing are provided, allowing sites to
retrieve sequences from remote databases via the Web or build in-house
...
databases held at NCBI, and others, using HTTP as a protocol. See
L<Bio::DB::GenBank> and L<Bio::DB::GenPept> for more information.
...
the Bio::DB::BioSeqI is expected. See L<Bio::DB::Fasta>, for example.

Flat file indexing of Fasta files is also provided by Bio::DB::Fasta,
please see L<Bio::DB::Fasta> for more information.
...
Currently only mysql is supported, but other RDBMs are planned.
...
bpfetch.pl has other options to connect to Genbank across the network.
...
or any other directory
   ...
   setenv BIOPERL_INDEX

(or export in bash) in the system login script to /nfs/datadisk/bioperlindex/
...
Download and install the bioperl-db package
(ftp://bioperl.org/pub/DIST/bioperl-db-0.1.tar.gz). The current version
relies on the prior installation of the free mysql server
(http://www.mysql.com). The bioperl-db package integrates with Bioperl's
objects and contains tables for sequences, entries, sequence features,
taxonomic information, references, keywords, a wide variety of genetic
markers, and more. With a Seq object and a built-in database "adaptor" one
can load the object's data in one step, as in 
...
and retrieve arrays of Seq objects. For example
  ...
  @seqobjs = $adaptor->fetch_by_query( -constraints =>
            ["or", 
                  ["and",
                         "species=Human",            # find all human
                         "keywords=topoisomerase*"], # topoisomerases
                  ["and",
                         "species like Escherichia*",# and E. coli
                         "keywords like *gyrase*"],  # DNA gyrases
            ]);
