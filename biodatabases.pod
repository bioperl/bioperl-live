...
Bioperl databases @@VERSION@@ - how to use databases with Bioperl
...
=item scripts/bpfetch.PLS

Fetches sequences from a Database

=item scripts/bpindex.PLS

Builds indexes for flat files databases which are easily accessible
by bpfetch.PLS. These scripts are discussed in more detail below.
...
SwissPfam, GenBank, or Blast files then the bpfetch.PLS and
bpindex.PLS scripts are great ways to start off (and also reading the
scripts shows you how to use the Bioperl indexing stuff). bpfetch.PLS 
and bpindex.PLS coordinate using two environment variables
...
set up, is to go
  ...
  bpindex.PLS <index-name> <filenames as full path>

eg, for Fasta files

  bpindex.PLS est /nfs/somewhere/fastafiles/est*.fa

Or, for EMBL/Swissprot files

  bpindex.PLS -fmt=EMBL swiss /nfs/somewhere/swiss/swissprot.dat

To retrieve sequences from the index go

  bpfetch.PLS <index-name>:<id>

eg,

  bpfetch.PLS est:AA01234

or

  bpfetch.PLS swiss:VAV_HUMAN


bpfetch.PLS also has other options to connect to Genbank across the network.
...
go 
   ...
   bpindex.PLS swissprot /nfs/datadisk/swiss/swissprot.dat

etc. You are now ready to use bpfetch.PLS. See L<Bio::Index::Fasta>, 
...
relies on the prior installation of a database server, such as mySQL from
...
BioSQL is joint project of the OpenBio languages, bioperl, biopython,
biojava, and bioruby. This project has created a single relational 
schema that's accessible using any of these languages. To use BioSQL
you'll need Bioperl, version 1.2 or later, bioperl-db from 
www.bioperl.org, and biosql. The biosql package can be obtained at 
www.open-bio.org. BioSQL currently supports 3 different database 
servers, mySQL, Postgres, and Oracle. Consult the bioperl-db and 
biosql packages for more details on installing and using BioSQL.
