...
You can also use Bioperl to retrieve sequences from remote databases
via the Web or to build and query in-house relational databases (RDBs).

Different scripts are provided to get you started with some of these
...
Fetches sequences from a flat-file database

=item scripts/bp_index.PLS

Builds indexes for flat files databases which are accessed
by bp_fetch.PLS. These scripts are discussed in more detail below.

=item scripts/DB/bioflat_index.PLS

Build indexes for flat file databases that can be used by the
OBDA sequence retrieval system.
...
L<Bio::DB::GenBank>, L<Bio::DB::GenPept>, and 
L<REMOTE SEQUENCE RETRIEVAL (Bio::DB::*)> for more information.
...
for more information, or the SeqIO HOWTO (http://bioperl.org/HOWTOs).
...
=item The OBDA system

Open Biological Database Access is a system for sequence retrieval
that wraps all the different data sources mentioned above together
...
http://cvs.open-bio.org/cgi-bin/viewcvs/viewcvs.cgi/bioperl-db/?cvsroot=bioperl
...
taxonomic information, references, keywords, and more. Using a
database adaptor factory, one can create "persistent objects" from
bioperl objects. Persistent objects know how to store, update, and
remove themselves to/from the database. For example,
...
There was a presentation at BOSC03 in Brisbane on bioperl-db,
including how to code several use cases. You may want to consult the
slides
L<http://www.open-bio.org/bosc2003/slides/Persistent_Bioperl_BOSC03.pdf>.

Similarly, one can query the database with id's or using query
objects.  See L<Bio::DB::Query::BioQuery> and
L<Bio::DB::Query::QueryConstraint> for examples.
...
The Open Biological Database Access (OBDA) system provides a single,
configurable retrieval interface to sequence databases regardless of their
format: relational (as in BioSQL), local indexed flat file, or remote and
Internet-accessible. The user creates a single configuration file, a
"registry", that describes the data source and OBDA handles the rest.
See the OBDA_Access HOWTO for more information
(http://bioperl.org/HOWTOs) or L<Bio::DB::Registry>.
