...
$soe->mark_instantiated(($ip_term1, $ip_term2, $ip_term3));

my @child_terms = $soe->get_child_terms($ip_term1);
ok (scalar(@child_terms), 1);
...
ok( $ancestor_terms[1], $ip_term1 );
ok (scalar(@ancestor_terms), 2);
ok (scalar($soe->get_ancestor_terms($ip_term3, $rel_type)), 2);
